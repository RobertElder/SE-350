...
int uart_init(int n_uart) {
	...
	pUart->IER = IER_Receive_Data_Available | IER_THR_Empty | IER_RLS; 
...
void c_UART0_IRQHandler(void)
{
	...
	if (IIR_IntId & IIR_RDA) {  // Receive Data Avaialbe
		...
		LPC_UART0->IER = IER_THR_Empty | IER_RLS;			// Disable RBR 
	    uart_send_string( 0, (uint8_t *)g_UART0_buffer, g_UART0_count );
	    g_UART0_count = 0;
	    LPC_UART0->IER = IER_THR_Empty | IER_RLS | IER_Receive_Data_Available;	// Re-enable RBR
...
void uart0_put_string(unsigned char * c){
	...
	LPC_UART0->IER = IER_THR_Empty | IER_RLS;			// Disable IER_Receive_Data_Available 
	...
	LPC_UART0->IER = IER_THR_Empty | IER_RLS | IER_Receive_Data_Available;	// Re-enable IER_Receive_Data_Available
}

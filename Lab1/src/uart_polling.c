...
int uart_polling_init(int n_uart) {
    ...
    LPC_UART_TypeDef *pUart;  // ptr to memory mapped device UART, check LPC17xx.h
	                          // for UART register C structure overlay
...
int uart_get_char(int n_uart)
{
	...
	while (!(pUart->LSR & LSR_RDR)); // polling the LSR RDR (Receiver Data Ready) bit to wait it is not empty
	return (pUart->RBR); 
}
...
int uart_put_char(int n_uart, unsigned char c)
{
	...
	//while (!(pUart->LSR &0x20));  // polling LSR:THRE so that it contains a chr (i.e. the bit is set to zero)
	while (!(pUart->LSR & LSR_THRE)); // polling LSR THRE bit to wait it is empty
